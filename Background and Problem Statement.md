...
- **Temporal signature**: The entire Hop 1 → Hop 2 sequence completes in under 65 minutes. In temporal graph analysis, this creates a directed **causal temporal path** $v_A \xrightarrow{t_0} v_{m_i} \xrightarrow{t_0 + \delta} v_{C_j}$ where $\delta < 40\ \text{minutes}$, a rapid relay that distinguishes laundering from legitimate multi-step transfers (which typically have inter-hop intervals of days to weeks).
